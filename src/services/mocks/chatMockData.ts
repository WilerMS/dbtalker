...
export const buildMockTextMessage = (text: string): CompleteMessage => {
  const data: TextData = { text }

  return buildBotMessage('text', data)
}

export const buildMockWidgetMessage = (
...
export const buildMockInitialMessages = (
  databaseId: string,
): CompleteMessage[] => {
  return [
    buildMockTextMessage(
      `KubePath listo para ${databaseId}. Pide un esquema, una metrica o una tabla y te respondo con un widget en el feed.`,
    ),
    buildMockWidgetMessage('schema'),
  ]
}

export const detectMockWidgetType = (
  query: string,
): PreviewWidgetType | null => {
  const q = query.toLowerCase()

  if (q.includes('schema') || q.includes('tabla')) return 'schema'
  if (q.includes('kpi') || q.includes('revenue') || q.includes('ingresos')) {
    return 'kpi'
  }
  if (q.includes('trend') || q.includes('line') || q.includes('tendencia')) {
    return 'line'
  }
  if (q.includes('bar') || q.includes('services') || q.includes('servicios')) {
    return 'bar'
  }
  if (q.includes('table') || q.includes('rows') || q.includes('filas')) {
    return 'table'
  }
...
export const buildMockStreamingTextData = (
  widgetType: PreviewWidgetType | null,
  databaseId: string,
): TextData => {
  if (widgetType) {
    return { text: loremIpsumPreviewText }
  }

  return {
    text: `Base ${databaseId} lista. Prueba con: "muestrame el schema", "ensename un KPI" o "quiero una tabla".`,
  }
}

export const buildMockPreviewTextMessage = (): CompleteMessage => {
  return buildMockTextMessage(loremIpsumPreviewText)
}
